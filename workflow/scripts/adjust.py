...
if __name__ == '__main__':
    ...
    if 'cat:bias_adjust_reference' not in ds_scen.attrs:
    ...
    if Path(output).suffix == '.zip':
        tmp_zarr_and_zip(ds_scen, output, rechunk=config['chunks']['working'], encoding={v: {"dtype": "float32"} for v in ds_tr.data_vars})
    else:
        xs.save_to_zarr(ds_scen, output, rechunk=config['chunks']['working'], encoding={v: {"dtype": "float32"} for v in ds_tr.data_vars})
