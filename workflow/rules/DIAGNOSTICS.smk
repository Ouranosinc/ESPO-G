...
rule concatenation_final:
    ...
    output:
        directory(Path(config['paths']['final'])/"FINAL/{region}/day_{sim_id}_{region}_1950-2100.zarr"),
    wildcard_constraints:
        region=r"[a-zA-Z]+_[a-zA-Z]+"
    log:
        "logs/concatenation_final_{sim_id}_{region}"
    script:
...
rule concatenation_diag:
    ...
    output:
        directory(Path(config['paths']['final'])/"diagnostics/{region}/{sim_id}/{level}_{sim_id}_{region}.zar")
    wildcard_constraints:
        region=r"[a-zA-Z]+_[a-zA-Z]+"
    log:
        "logs/concatenation_diag_{level}_{sim_id}_{region}"
    script:
