...
> $ chmod +x status-sacct.sh
 ...
 et ajouter `cluster-generic-status-cmd: status-sacct.sh` dans *config.v8+.yaml* et l'option `--parsable` sous `sbatch`. Il faut aussi bien choisir la valeur de `max-status-checks-per-second` qui correspond au nombre de fois maximum qu'on peut surveiller l'état de tous les jobs et non par job. C'est à dire que pour 500 jobs qui roulent en parallèle il faut au moins `max-status-checks-per-second: 50` pour vérifier l'état de chaque job au moins une fois.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk4OTQ0MDQ3OSw0OTM2OTU0MSwtMjE0MD
EwMzU4LDg3NzY3MTg0NiwtMTkwODY5MjYwMiwxOTc3NTEyNTEy
XX0=
-->