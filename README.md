...
<li><strong>off-properties_ESPO-G.yaml:</strong> fournie à <code>xs.properties_and_measures</code> présent dans les scripts des règles <em>off_diag_scen_prop_meas, off_diag_sim_prop_meas</em> et <em>off_diag_ref_prop</em>, un chemin d’accès à un fichier YAML qui indique comment calculer les  propriétés.</li>
...
<li>
<p><strong>sbatch --partition:</strong> pour avoir la priorité Ouranos</p>
</li>
<li>
<p><strong>sbatch --account:</strong>  choisir un compte. Obligatoire sur narval</p>
</li>
<li>
<p><strong>sbatch --constraint:</strong> pour accéder à bébé narval</p>
</li>
<li>
<p><strong>sbatch --cpus-per-task:</strong> nombre de workers de dasks</p>
</li>
<li>
<p><strong>sbatch --job-name:</strong>  renommer le nom de la job en fonction de du nom de la règle et des ses wildcards</p>
</li>
<li>
<p><strong>sbatch --output:</strong> renommer le nom du fichier output en fonction de du nom de la règle et des ses wildcards</p>
</li>
<li>
<p><strong>sbatch --parsable:</strong> pour que sbatch renvoie uniquement le job ID sans aucun texte supplémentaire. Est obligatoire si on utilise<br>
cluster-generic-status-cmd.</p>
</li>
<li>
<p><strong>cluster-generic-status-cmd:</strong> permet d’annuler les jobs slurm lorsque le processus de snakemake est interrompu</p>
</li>
<li>
<p><strong>restart-times:</strong> exécute jusqu’à 3 fois la règle en cas d’échec.</p>
</li>
<li>
<p><strong>max-jobs-per-second:</strong>  Cela peut être utile pour éviter de surcharger le cluster avec trop de soumissions simultanées.</p>
</li>
<li>
<p><strong>max-status-checks-per-second:</strong> le nombre total de vérification d’état de tous les jobs</p>
</li>
<li>
<p><strong>latency-wait:</strong> attendre un certain temps pour que les fichiers soient visibles en cas de MissingFileExeption error</p>
</li>
<li>
<p><strong>jobs:</strong> le maximum de jobs qui peuvent être roulés en parallèle</p>
</li>
<li>
<p><strong>keep-going:</strong>  pour continuer à exécuter les règles qui sont independantes d’une règle qui a échoué</p>
</li>
<li>
<p><strong>rerun-incomplete:</strong>  permet de relancer les tâches qui n’ont pas été complètement exécutées lors d’une précédente exécution</p>
</li>
<li>
<p><strong>printshellcmds:</strong>  pour que snakemake affiche les commandes shell qui sont exécutées pour chaque règle. Cela peut être très utile pour<br>
le  débogage.</p>
</li>
</ul>
...
vérifier.</li>
<li>Les wildcards ne peuvent pas contenir le caractère <strong>“/”</strong> si vous voulez les<br>
...
<pre><code>$ snakemake --dag all &gt; dag.dot 
</code></pre>
<p>puis sur doris il faudrait installer graphviz et ses exécutables avec</p>
<pre><code>$ mamba install graphviz 
</code></pre>
<p>et faire</p>
<pre><code>$ dot -Tpng  -Gratio=fill dag.dot -o dag.png  
</code></pre>
<p><strong>-Tpng</strong>: spécifie que la sortie doit être au format PNG. On peut remplacer png par svg ou pdf.<br>
<strong>-Gratio</strong>:ajuste le ratio de l’image pour remplir l’espace disponible.</p>
<p><strong>TO DO:</strong> il est possible de télècharger une version compilée de graphviz et de le load en tant que module avec</p>
<pre><code>$ module load graphviz 
</code></pre>
<p>et il suffira seulement de faire la commande suivante pour générer le dag:</p>
<pre><code>$ snakemake --dag all | dot -Tpng dag.png  
</code></pre>
<p>ou</p>
<pre><code>$ snakemake --dag &lt;rule_name&gt; | dot -Tpng dag.png  
</code></pre>
...
<p>Par défaut le <strong>panoptes</strong> écoute sur le <strong>port 5000</strong>, vous pouvez le modifier au besoin.<br>
Snakemake envoie les requêtes suivantes à wms monitor :</p>
<p><strong>GET /api/service-info</strong> : Snakemake obtient le statut de panoptes. Snakemake continue de fonctionner si le statut (json[‘status’]) est ‘running’. Dans tous les autres cas, snakemake se retire avec un message d’erreur.<br>
<strong>GET /create_workflow</strong> : Snakemake obtient un id/name unique str(uuid.uuid4()) pour chaque workflow déclenché.<br>
<strong>POST /update_workflow_status</strong> : Snakemake publie des mises à jour pour les flux de travail/travaux. Le dictionnaire envoyé contient le dictionnaire des messages de journal, l’horodatage actuel et l’identifiant/nom unique du flux de travail.</p>
<pre><code>{
    'msg': repr(msg),
