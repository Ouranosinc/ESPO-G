...
if __name__ == "__main__":
    ...
    for f in glob.glob(f"{CONFIG['paths']['final']}/preswap/*"):
        if "CRCM5" not in f:
            print(f)
            ds = xr.open_zarr(f)
            ...
            if not Path(
                ...
                mask = ds.dtr >= 0
                path = f"{CONFIG['paths']['final']}/staging/simulation/biasadjusted/ESPO_v20_CaSR/CMIP6/ScenarioMIP/NAM/*/{source}/{experiment}/{member}/day/"
                print(path)
                ...
                xs.save_to_zarr(
                    ds_tasmax_new,
                    f"{final_path}/tasmax_{source}_{experiment}_{member}.zarr.zip",
                    zip_zarrdir="${SLURM_TMPDIR}",
                )
                xs.save_to_zarr(
                    ds_tasmin_new,
                    f"{final_path}/tasmin_{source}_{experiment}_{member}.zarr.zip",
                    zip_zarrdir="${SLURM_TMPDIR}",
                )
                xs.save_to_zarr(
                    diff_tasmax,
                    f"{final_path}/diff_tasmax_{source}_{experiment}_{member}.zarr.zip",
                    zip_zarrdir="${SLURM_TMPDIR}",
                )
                xs.save_to_zarr(
                    diff_tasmin,
