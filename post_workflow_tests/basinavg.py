...
if __name__ == "__main__":
    ...
    for dm in ["MPI-ESM1-2-LR", "CanESM5", "NorESM2-MM"]:
        ds_adj = xr.open_mfdataset(
            glob.glob(
                f"{CONFIG['DQM-dtr']}/staging/simulation/biasadjusted/*_v20_CaSR/CMIP6/CORDEX/NAM/OURANOS/CRCM5-SN/{dm}/*/ssp370/r1/day/*/*.zarr.zip"
            ),
            engine="zarr",
        )
        ...
        for gdfs, geoname in zip([gdfsA, gdfs6], ["atlas", "lev06"]):
            ...
            for j in range(len(gdfs)):
                if not Path(
                    f"{CONFIG['pathind']}/bilanpr/bassinextremes_{geoname}_{j}_{dm}.csv"
                ).exists():
                    print(j)
                    l = []
                    for ds, method in zip(
                        [ds_ref, ds_adj, ds_ext], ["CaSR", "adj", "ext"]
                    ):
                        print(method)
                        print(ds.coords)
                        ...
                        cur = xs.spatial_mean(
                            ...
                            method="xesmf",
                            region=dict(
                                name="tmp", method="shape", shape=gdfs.iloc[[j]]
                            ),
