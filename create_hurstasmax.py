...
if __name__ == '__main__':
    ...
    ds= cat.search(source='ERA5-Land', variable=['tas','tdps'], frequency='1hr').to_dataset()
    ...
    for i in range(151):
        if not os.path.exists(f"{CONFIG['tmppath']}{i}.zarr"):
            ...
            xs.save_to_zarr(outcur,f"{CONFIG['tmppath']}{i}.zarr" )

    # merge the 5 parts
    files= [xr.open_zarr(f"{CONFIG['tmppath']}{i}.zarr") for i in range(151)]
